...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PREPARE_STATEMENT(WORLD_LOAD_NPCGUARD_TEMPLATES, "SELECT `entry`,`type`,`value`,`distance`,`teleId`,`comment` FROM `npc_areaguard_template`", CONNECTION_SYNCH);
    PREPARE_STATEMENT(WORLD_ADD_NPCGUARD_TEMPLATE, "REPLACE INTO `npc_areaguard_template`(`entry`,`type`,`value`,`distance`,`teleId`,`comment`) VALUES (?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    PREPARE_STATEMENT(WORLD_DEL_NPCGUARD_TEMPLATE, "DELETE FROM `npc_areaguard_template` WHERE `entry` = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(WORLD_LOAD_NPCGUARDS, "SELECT `guid`,`guardEntry` FROM `npc_areaguard`", CONNECTION_SYNCH);
    PREPARE_STATEMENT(WORLD_ADD_NPCGUARD, "REPLACE INTO `npc_areaguard`(`guid`,`guardEntry`) VALUES (?, ?)", CONNECTION_ASYNC);
    PREPARE_STATEMENT(WORLD_DEL_NPCGUARD, "DELETE FROM `npc_areaguard` WHERE `guardEntry` = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(WORLD_DEL_NPCGUARD_BY_GUID, "DELETE FROM `npc_areaguard` WHERE `guid` = ?", CONNECTION_ASYNC);
}
