...
void WardenMac::HandleData(ByteBuffer &buff)
{
    ...
    {
        sLogMgr->WriteLn(WARDEN_LOG, LOGL_WARNING, "Handle data failed: SHA1 hash is wrong!");
        found = true;
    }
    ...
    {
        sLogMgr->WriteLn(WARDEN_LOG, LOGL_WARNING, "Handle data failed: MD5 hash is wrong!");
        found = true;
    }
